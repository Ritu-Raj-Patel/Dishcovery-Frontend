...
from lib.models import MatchFilters, MatchRequest, ScoredRecipe
...
def test_pydantic_validation():
    ...
    try:
        ...
        for scored_recipe in scored_recipes:
            try:
                validated = ScoredRecipe(**scored_recipe)
                validated_recipes.append(validated)
            except Exception as e:
                print(f"Validation error for recipe: {e}")
                print(f"Recipe data: {scored_recipe}")
        
        print(f"Validated {len(validated_recipes)} recipes")
        
        # Paginate results
        paginated = paginate(validated_recipes, request.page or 1, request.limit or 20)
